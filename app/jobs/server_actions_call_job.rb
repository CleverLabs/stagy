...
class ServerActionsCallJob < ApplicationJob
  def perform(class_name, configurations, build_action)
    deserialized_configurations = configurations.map { |configuration| Deployment::Configuration.new(configuration).add_private_key(build_action.project_instance.project.private_key) }
