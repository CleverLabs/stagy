# frozen_string_literal: true

module Github
  module WebhookProcessors
    class RemoveRepo
      def initialize(body)
        @wrapped_body = Github::Events::Installation.new(payload: body)
        @project = ::Project.find_by(integration_type: Github::User::PROVIDER, integration_id: @wrapped_body.installation_id)
      end

      def call
        repos_ids = @wrapped_body.removed_repos.map(&:id)
        ::DeploymentConfiguration.where(project: @project, integration_type: Github::User::PROVIDER, integration_id: repos_ids).update_all(status: DeploymentConfigurationConstants::REMOVED)
        ReturnValue.ok(nil)
      end
    end
  end
end
